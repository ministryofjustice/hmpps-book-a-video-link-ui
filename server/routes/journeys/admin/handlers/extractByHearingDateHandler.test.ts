...
beforeEach(() => {
  ...
  videoLinkService.downloadBookingDataByHearingDate.mockImplementation((agencyType, date, daysToExtract, res) => {
    return new Promise(() => {
...
describe('Extract by hearing date handler', () => {
  describe('GET', () => {
    it(`should render the correct view`, () => {
      return request(app)
        .get(`/admin/extract-by-hearing-date`)
        .expect('Content-Type', /html/)
        .expect(res => {
          const $ = cheerio.load(res.text)
          const heading = $('h1').text().trim()

          expect(heading).toBe(`Extract summary data by hearing date`)
          expect(auditService.logPageView).toHaveBeenCalledWith(Page.EXTRACT_BY_HEARING_PAGE, {
            who: user.username,
  ...
  describe('POST', () => {
    it(`should validate empty form`, () => {
      return request(app)
        .post(`/admin/extract-by-hearing-date`)
    ...
    it.each(['court', 'probation'])(`should download a %s CSV`, (type: string) => {
      return request(app)
        ...
        .expect(res => {
          ...
          expect(videoLinkService.downloadBookingDataByHearingDate).toHaveBeenCalledWith(
            type,
            startOfToday(),
            7,
            expect.anything(),
